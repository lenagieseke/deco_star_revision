...
\textbf{Design space} refers loosely to all visual results a technique can create. For example, Perlin noise\mf{citation required?} has a rather restricted design space of noise images, only differing, for example, in their frequency\mf{is that the only parameter? Otherwise, we could phrase this more weakly and still make the point}. Drawing with a pen can result in many different designs, thus resulting in a larger design space.
...
\textbf{Procedural} refers to the production of output by running an algorithm, \revremove{}{or}\rev{}{, such as} a rule-based system.
...
\mf{I think "implicit equation" is too restrictive here ↑. I think we should drop that and instead write:}

\rev{}{
A \textbf{Parameterized} system offers separately controllable characteristics. In our use of the term, it does not necessarily imply a procedural pattern representation, but could also \eg{} apply to a data-driven one.
}
\mf{... okay?}

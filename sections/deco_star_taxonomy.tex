...
\noindent\textbf{Where (Canvas)} does the input have an effect and what is the area of influence? 
...
Input can be given \textit{before} the creation process, \textit{during} it when parts of the results are already visible, or \textit{after}, when the result is visible and can be adjusted retrospectively.
...
\begin{itemize}
    ...
    \item \textit{Curves}: A one-dimensional curve or path to be \rev{}{followed with elements of the pattern}. The curve is given as a whole before the generation starts.

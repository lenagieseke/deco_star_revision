...
\begin{itemize}
    ...
    \item \textit{Separate UI}: A separate UI is given through which an artist gives input and activates states. Separate UIs are often in close proximity to the canvas, carefully designed and easily usable. However, because they detach the work from the actual output, separate UIs have an abstract nature 
    % \rev{}{of indirection} lg: I don't understand this phrasing
...
\noindent\textbf{Who (User)} describes the input in regard to the needed type of skill set. 
\begin{figure}[H]
    \centering
...
This category can be in part derived from the above characteristics of \textit{how} and \textit{what}. In most general terms, this category can be classified as the skill set of a \textit{Programmer}, for giving, \eg~code as input, requiring analytical-formal and logical thinking and the ability to abstract. On the other end of the spectrum, the skill set of an \textit{Artist} is needed, \eg~for drawing, requiring intuitive-visual and spatial thinking and the ability to create. The \textit{who} category is listed here for completeness. The discussion of needed competencies, skills and mindsets, including the accompanying psychological and artistic aspects, is out of the scope of this survey, though.
...
\begin{itemize}
    ...
    \item \textit{Sketch-Based}: Sketches and curves \rev{Better explanation, review 5}{defining indirect control elements} on the canvas - for example, the drawing of a mask with a pen tool.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textit{Curves to Fill}: A one-dimensional curve or path to be \rev{}{filled with elements of the pattern}. The curve is given as a whole before the filling starts.
    % \mf{Can you actually “fill” a (1D) curve? Should we say: “follow a curve” instead?} LG: I would prefer to leave is as "filling" as "following" doesn't fit into the text well.
\end{itemize}
